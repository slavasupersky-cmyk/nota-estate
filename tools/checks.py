...
src='каталоги, декларации, сайты застройщиков (база 23.09)'
new=[]
...
for x in r:
    ...
    u,pk=num(x['units_total']),num(x['parking'])
    if u: add(s,'04','да' if u<=UN[cl] else 'нет',f'{int(u)} квартир',src,'квартир на этаже — не собрано')
...
if len(sys.argv)>1:
    ...
    open(B+'proverki.csv','w',encoding='utf-8').write('﻿'+buf.getvalue()); print('записано',len(new))
